...
{
    [TestClass]
    public class GetIncidentByIdTest
    {
        [TestMethod]
        public void GetById_AddANewIncidentAndRetrieveIt_ReturnIncidentNotNull()
        {
            //ARRANGE
            ...
            context.SaveChanges();
            //ACT
            var result = incidentRepository.GetById(addedIncident.Id);
            //ASSERT
            Assert.IsNotNull(result);
            Assert.AreEqual("No coffee", result.Description);
            Assert.AreEqual(IncidentStatus.Waiting, result.Status);
        }
    }
}